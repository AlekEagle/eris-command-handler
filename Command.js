...
class Command {
   ...
   * @arg {RegExp} [options.whitespaceSeparator=/\s/g] The regex expression to split arguments.
   */
  constructor(label, generator, options, parentCommand) {
    ...
    this.whitespaceSeparator = options.whitespaceSeparator;
    ...
    this.removeWhitespace =
      options.removeWhitespace === undefined ? true : options.removeWhitespace;
          ...
          if (typeof button.response === "string") {
            ...
            button.responses = button.response.map((item, otherIndex) => {
              if (typeof item === "string") {
                ...
                throw new Error(
                  `Invalid reaction button response generator (index ${index}:${otherIndex})`
                );
              }
            });
            button.execute = () =>
              button.responses[
                Math.floor(Math.random() * button.responses.length)
              ]();
            return button;
            ...
            throw new Error(
              `Invalid reaction button response generator (index ${index})`
            );
          }
        })
      : null;
    ...
    if (typeof generator === "string") {
      ...
      this.responses = generator.map((item, index) => {
        if (typeof item === "string") {
          ...
          throw new Error(
            `Invalid command response generator (index ${index})`
          );
        }
      });
      this.execute = () =>
  ...
  get fullLabel() {
    return `${this.parentCommand ? this.parentCommand.fullLabel + " " : ""}${
      this.label
    }`;
  }
  ...
  cooldownExclusionCheck(msg) {
    return (
      ...
      (msg.channel.guild &&
        this.cooldownExclusions.guildIDs.includes(msg.channel.guild.id))
    );
  }
  ...
  async permissionCheck(msg) {
    ...
    if (this.requirements.userIDs) {
      const userIDs =
        typeof this.requirements.userIDs === "function"
          ? await this.requirements.userIDs(msg)
          : this.requirements.userIDs;
    ...
    if (msg.channel.guild) {
      ...
      if (this.requirements.permissions) {
        const requiredPermissions = Object.keys(
          typeof this.requirements.permissions === "function"
            ? await this.requirements.permissions(msg)
            : this.requirements.permissions
        );
      ...
      if (this.requirements.roleIDs) {
        const requiredRoleIDs =
          typeof this.requirements.roleIDs === "function"
            ? await this.requirements.roleIDs(msg)
      ...
      if (this.requirements.roleNames) {
        const roleNames = roleIDs.map(
          (roleID) => msg.channel.guild.roles.get(roleID).name
        );
        const requiredRoleNames =
          typeof this.requirements.roleNames === "function"
            ? await this.requirements.roleNames(msg)
  ...
  async process(args, msg) {
    const shouldDelete =
      this.deleteCommand &&
      msg.channel.guild &&
      msg.channel.permissionsOf(msg._client.user.id).has("manageMessages");
    ...
    if (this.cooldown !== 0 && !this.cooldownCheck(msg)) {
      ...
      if (
        ...
        (!this.cooldownReturns ||
          this.cooldownAmounts[msg.author.id] <= this.cooldownReturns)
      ) {
        reply =
          typeof this.cooldownMessage === "function"
            ? await this.cooldownMessage(msg)
            : this.cooldownMessage;
        if (reply) {
    ...
    if (!(await this.permissionCheck(msg))) {
      ...
      reply =
        typeof this.permissionMessage === "function"
          ? await this.permissionMessage(msg)
    ...
    if (args.length === 0) {
      ...
      if (this.argsRequired) {
        ...
        reply =
          typeof this.invalidUsageMessage === "function"
            ? await this.invalidUsageMessage(msg)
        ...
        if (reply) {
          ...
          if (reply.content) {
            reply.content = reply.content
              ...
              .replace(/%label%/g, this.fullLabel);
          }
          msg.channel.createMessage(reply);
        }
        return;
    ...
    let subcommand;
    if (
      (subcommand = this.subcommands[label]) !== undefined ||
      ((subcommand = this.subcommands[label.toLowerCase()]) !== undefined &&
        subcommand.caseInsensitive)
    ) {
  ...
  registerSubcommand(label, generator, options = {}) {
    ...
    for (const key in this.defaultSubcommandOptions) {
      if (
        this.defaultSubcommandOptions.hasOwnProperty(key) &&
        options[key] === undefined
      ) {
        options[key] = this.defaultSubcommandOptions[key];
        options.defaultSubcommandOptions[key] =
          this.defaultSubcommandOptions[key];
      }
    }
  ...
  unregisterSubcommand(label) {
    ...
    if (original) {
      this.subcommands[original].aliases.splice(
        this.subcommands[original].aliases.indexOf(label),
        1
      );
