...
class CommandClient extends Client {
   ...
   * @arg {RegExp} [commandOptions.whitespaceSeparator=/\s/g] The regex expression to split arguments.
   */
    ...
    this.once('shardPreReady', () => {
      ...
      if (Array.isArray(this.commandOptions.prefix)) {
        for (let i = 0; i < this.commandOptions.prefix.length; ++i) {
          this.commandOptions.prefix[i] = this.commandOptions.prefix[i].replace(
            /@mention/g,
            this.user.mention
          );
        }
      } else {
        this.commandOptions.prefix = this.commandOptions.prefix.replace(
          /@mention/g,
          this.user.mention
        );
      }
      for (const key of Object.keys(this.guildPrefixes)) {
        if (Array.isArray(this.guildPrefixes[key])) {
          for (let i = 0; i < this.guildPrefixes[key].length; ++i) {
            this.guildPrefixes[key][i] = this.guildPrefixes[key][i].replace(
              /@mention/g,
              this.user.mention
            );
          }
        } else {
          this.guildPrefixes[key] = this.guildPrefixes[key].replace(
            /@mention/g,
            this.user.mention
          );
        }
          ...
          if (args.length > 0) {
            ...
            if (subcommands.length > 0) {
              ...
              for (const subLabel of subcommands) {
                if (
                  ...
                  cur.subcommands[subLabel].permissionCheck(msg)
                ) {
            ...
            for (const label in this.commands) {
              if (
                ...
                this.commands[label] &&
                this.commands[label].permissionCheck(msg) &&
                !this.commands[label].hidden
              ) {
  ...
  async onMessageCreate(msg) {
    ...
    msg.command = false;
    if (
      (!this.commandOptions.ignoreSelf || msg.author.id !== this.user.id) &&
  ...
  async onMessageReactionEvent(msg, emoji, userID) {
    if (
      !this.ready ||
      userID === this.user.id ||
      !(msg.content || msg.embeds || msg.attachments)
    ) {
      return;
    }
  ...
  registerCommand(label, generator, options = {}) {
    ...
    if (
      this.commands[label] ||
      (this.commands[lowercaseCommand] &&
        this.commands[lowercaseCommand].caseInsensitive)
    ) {
    ...
    for (const key in this.commandOptions.defaultCommandOptions) {
      if (
        this.commandOptions.defaultCommandOptions.hasOwnProperty(key) &&
        options[key] === undefined
      ) {
        options[key] = this.commandOptions.defaultCommandOptions[key];
        options.defaultSubcommandOptions[key] =
          this.commandOptions.defaultCommandOptions[key];
      }
    }
    ...
    if (options.aliases) {
      options.aliases.forEach(alias => {
        ...
        if (
          ...
          throw new Error(
            `You have already registered a command for alias ${alias}`
          );
        }
  ...
  unregisterCommand(label) {
    ...
    if (original) {
      this.commands[original].aliases.splice(
        this.commands[original].aliases.indexOf(label),
        1
      );
